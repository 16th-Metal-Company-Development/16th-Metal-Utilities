...
if !OPTRE_Hud_On then {
	if !_brokenBool then { // Not Broken 
	  ...
	  //OPTRE_Hud_AmmoCurrent = (getText (configfile >> "CfgOptreHudSchemes" >> _hudStyle >> "ammoCounter"));//"OPTRE_Marrine_2_AmmoCount"
		OPTRE_Hud_LHDCurrent = ((getArray (configfile >> "CfgOptreHudSchemes" >> _hudStyle >> "LHDs" >> ("LHD_" + (str OPTRE_LHD_Function)))) select 0);
	  ...
	  //OPTRE_Hud_OverSheildBar = (getText (configfile >> "CfgOptreHudSchemes" >> _hudStyle >> "overSheildBar"));
		...
		//make sure the "visor off" helmet is being worn.
		if _hel then {
			if (([(_varHelmetArray select 0), _helmet, true] call BIS_fnc_inString)) then {
	...
	if _hel then {
		if !_brokenBool then {
			...
			//if the helmet uses a prefix for "polariezd" / "hud on", then make sure we're wearing that helmet before switching
			if (_varHelmetArray select 1 != "") then {
				if (([(_varHelmetArray select 1), _helmet, true] call BIS_fnc_inString)) then {
					removeHeadgear player;
					player addHeadgear ([_helmet, (_varHelmetArray select 1), (_varHelmetArray select 0)] call CBA_fnc_replace);
				};
			} else { // if the helmet does NOT use a prefix for hud on, check we're not wearing the hud off version.
				if (!([(_varHelmetArray select 0), _helmet, true] call BIS_fnc_inString)) then {
					removeHeadgear player;
					player addHeadgear (_helmet + (_varHelmetArray select 0));
				};
			};
		};
	};
